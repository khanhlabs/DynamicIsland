...
function App() {
  ...
  useEffect(() => {
    ...
    void isEnabled().then(setAutostart).catch(() => undefined);
    const storedPosition = readStored<{ x: number; y: number } | null>("dynamic-island-position", null);
    if (storedPosition) void appWindow.setPosition(new PhysicalPosition(storedPosition.x, storedPosition.y));
    let unlistenMoved: (() => void) | undefined;
    void appWindow.onMoved(({ payload }) => {
      localStorage.setItem("dynamic-island-position", JSON.stringify({ x: payload.x, y: payload.y }));
  ...
  }, [breakMinutes, focusMinutes, pomodoro.endAt, pomodoro.mode, pomodoro.running, setOpen, showNotice]);
  ...
  const mediaProgress = useMemo(() => !media || media.durationSeconds <= 0
    ? 35 : Math.min(100, (media.positionSeconds / media.durationSeconds) * 100), [media]);
  ...
  const collapsedLabel = pomodoro.running
    ...
    : media?.title || "Hyperfocus | Lock In for 2 Hours | Deep Work Stu...";
                ...
                <article className="media-card">
                  ...
                  <div className="media-info">
                    <div className="media-source-tag">
                      <EdgeIcon size={14} />
                      <span>{media ? appName(media.source) : "Microsoft Edge"}</span>
                    </div>
                    <strong className="media-title">
                      {media?.title || "Hyperfocus | Lock In for 2 Hours | Deep Work Stu..."}
                    </strong>
                    <small className="media-artist">
                      {media?.artist || "Work With Paul"}
                    </small>
                    <div className="media-progress">
                      <i style={{ width: `${mediaProgress}%` }} />
                    </div>
                    <div className="media-times">
                      <span>{media ? formatMediaTime(media.positionSeconds) : "42:17"}</span>
                      <span>{media ? formatMediaTime(media.durationSeconds) : "2:00:00"}</span>
                    </div>
                  </div>
                  <div className="media-controls">
                    <button aria-label="Bài trước" disabled={!media} onClick={() => void runCommand("media_control", { action: "previous" })}>
                      <SkipBack size={18} />
                    </button>
                    <button
                      className="play"
                    ...
                    <button aria-label="Bài tiếp theo" disabled={!media} onClick={() => void runCommand("media_control", { action: "next" })}>
                      <SkipForward size={18} />
                    </button>
                  </div>
                </article>
